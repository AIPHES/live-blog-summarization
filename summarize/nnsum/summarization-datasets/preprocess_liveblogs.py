        ...
        for doc in documents:
            if 'is_key_event' in doc:
                ...
                try:
                    ...
                    pass
    if corpus == "guardian":        
        return list(reversed(doc_data)), "\n".join(summary), doc_id, genre, post_ids, post_times, url, title
    if corpus == "bbc":
        return list(reversed(doc_data)), "\n".join(summary), doc_id, genre, post_ids, post_times, url, title
    
def prepare_example(article_text, abstract_text, doc_id, genre, post_ids, post_times, url, title):
    global nlp
    inputs = []
    #print("Doc_id" , doc_id)
    ind = -1
